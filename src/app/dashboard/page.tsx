...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
        ...
        <div className="pt-8 pb-6 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <GreetingIcon />
              <p className="text-foreground/30 font-black text-[10px] uppercase tracking-[0.3em]">{getGreeting()}</p>
            </div>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-primary/5 text-primary rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all shadow-sm active:scale-95 border border-primary/10"
          >
      ...
      <div className="bg-gradient-to-br from-primary via-primary to-primary-dark rounded-[2.5rem] p-8 text-white shadow-lg shadow-primary/20 relative overflow-hidden group">
        ...
        <div className="grid grid-cols-2 gap-6 relative z-10">
          <div className="bg-white/10 rounded-[1.5rem] p-4 backdrop-blur-md border border-white/10">
          ...
          <div className="bg-white/10 rounded-[1.5rem] p-4 backdrop-blur-md border border-white/10">
            ...
            <p className="text-3xl font-black text-green-300">RM {todayCollection.toFixed(2)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-card to-muted/30 rounded-[2rem] p-6 border border-primary/5 shadow-sm">
          <p className="text-[10px] font-black text-foreground/30 uppercase tracking-[0.2em] mb-2">Monthly Sales</p>
          <div className="flex items-center gap-3">
            <div className="p-2 bg-green-500/10 rounded-xl">
              <TrendingUp className="w-4 h-4 text-green-500" />
            </div>
            <p className="text-2xl font-black text-primary tracking-tight">RM {monthlyRevenue.toLocaleString()}</p>
          </div>
        </div>
        <div className="bg-gradient-to-br from-card to-muted/30 rounded-[2rem] p-6 border border-primary/5 shadow-sm flex flex-col justify-center items-center gap-3">
          <Link href="/dashboard/planner" className="w-full text-center text-[10px] font-black uppercase tracking-widest text-primary bg-primary/5 px-4 py-3 rounded-2xl hover:bg-primary hover:text-white transition-all border border-primary/10 flex items-center justify-center gap-2">
      ...
      <div className="bg-card rounded-[2.5rem] p-8 border border-primary/5 shadow-md">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-black text-foreground flex items-center gap-3">
            Today&apos;s Agenda
            <span className="bg-primary/5 text-primary text-[10px] px-3 py-1 rounded-full uppercase font-black tracking-widest border border-primary/10">Timeline</span>
          </h2>
        </div>

        {schedule.length === 0 ? (
          <div className="text-center py-16 bg-muted/20 rounded-[2rem] border-2 border-dashed border-primary/10">
            <CheckCircle2 className="w-12 h-12 text-primary/20 mx-auto mb-3" />
          ...
          <div className="space-y-4">
            {schedule.slice(0, 3).map((item: any, idx: number) => (
              <div key={idx} className="flex items-center justify-between py-3 border-b border-primary/5 last:border-0 group">
                <div className="flex items-center gap-5">
                  <div className="h-12 w-12 rounded-2xl bg-primary/5 flex flex-col items-center justify-center border border-primary/10 group-hover:bg-primary transition-colors">
                  ...
                  <div>
                    <p className="font-black text-foreground tracking-tight group-hover:text-primary transition-colors">{item.product.name}</p>
            ...
            {schedule.length > 3 && (
              <Link href="/dashboard/planner" className="block text-center text-[10px] font-black uppercase tracking-[0.3em] text-primary/40 hover:text-primary transition-all pt-4">
      ...
      {pendingOrders.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-xl font-black text-foreground flex items-center gap-3">
            ...
            Needs Attention
            <span className="bg-red-500 text-white text-[10px] font-black px-2.5 py-0.5 rounded-full tracking-tighter shadow-sm">{pendingOrders.length}</span>
          </h2>
          <div className="space-y-4">
      ...
      {pendingOrders.length === 0 && productionOrders.length === 0 && schedule.length === 0 && (
        <div className="text-center py-20 space-y-4">
          <div className="relative inline-block">
            <CheckCircle2 className="w-16 h-16 text-primary opacity-10 mx-auto" />
            <motion.div 
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
          ...
          <p className="text-foreground/40 font-bold uppercase tracking-[0.2em] text-xs">All clear! No pending tasks.</p>
        </div>
      )}
    </div>
  );
